...
import sonidos from "../assets/sonidos/game.mp3";
...
const defaultHeroCard = {
  ...
  descriptionSuffix:
    "porque has jugado juegos etiquetados con Carreras, Simulador de Automóviles, Conducción y Deportes.",
  price: "$U2.300",
  mainImage:
  ...
  return (
    <article className="hero-card card border-0 rounded-4 overflow-hidden shadow-lg">
      <div className="hero-card__media position-relative p-0">
        <img
          className="hero-card__image w-100 h-100 object-fit-cover"
          src={mainImage}
          alt={title}
        />
      ...
      <div className="hero-card__content d-flex flex-column gap-3">
        ...
        <div
          className="hero-card__gallery row row-cols-2 g-2"
          aria-label={`Imágenes de vista previa de ${title}`}
        >
        ...
        <p className="hero-card__description mb-0">
          <span className="hero-card__highlight">{descriptionPrefix}</span>{" "}
          {descriptionSuffix}
        </p>

        <div
          className="hero-card__tags d-flex flex-wrap gap-2"
          aria-label="Etiquetas del juego"
        >
          {tags.map((tag) => (
            <span
              key={tag}
              className="hero-card__tag badge rounded-pill text-bg-secondary"
            >
              {tag}
            </span>
          ))}
        ...
        <div className="hero-card__footer d-flex align-items-center justify-content-between gap-3 mt-auto">
          ...
          <button
            className="hero-card__cta btn btn-light fw-bold"
            type="button"
